...
cleanup() {
    ...
    iptables -D INPUT -i tailscale0 -p udp -s "$edge_ts" -d "$office_ts" --dport "$wireguard_port" -j ACCEPT 2>/dev/null || true
    iptables -D INPUT -i "$tunnel" -s "$edge_tunnel_ip" -p tcp -m multiport --dports 18082,5223 -j ACCEPT 2>/dev/null || true
    iptables -D INPUT -i "$tunnel" -s "$edge_tunnel_ip" -p icmp --icmp-type echo-request -j ACCEPT 2>/dev/null || true
...
cleanup
iptables -I INPUT 1 -i tailscale0 -p udp -s "$edge_ts" -d "$office_ts" --dport "$wireguard_port" -j ACCEPT
ip link add "$tunnel" type wireguard
umask 077
printf '%s\n' "$wireguard_private_key" > "$key_file"
wg set "$tunnel" private-key "$key_file" listen-port "$wireguard_port" peer "$wireguard_peer_public_key" allowed-ips 0.0.0.0/0 endpoint "$edge_ts:$wireguard_port" persistent-keepalive 15
...
iptables -I INPUT 1 -i "$tunnel" -s "$edge_tunnel_ip" -p icmp --icmp-type echo-request -j ACCEPT
iptables -I INPUT 1 -i "$tunnel" -s "$edge_tunnel_ip" -p tcp -m multiport --dports 18082,5223 -j ACCEPT
